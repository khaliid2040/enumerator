...
int memory_info() {
    ...
    fclose(fp);
        char *unit;
        human_total_mem= convert_size(total_mem,&unit);
        human_free_mem= convert_size(free_mem,&unit);
        human_buffer= convert_size(buffer_mem,&unit);
        human_avialable= convert_size(available_mem,&   unit);
